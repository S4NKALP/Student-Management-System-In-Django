...
def dashboard(request):
    ...
    today_present = AttendanceRecord.objects.filter(
        attendance__date=today, student_attend=True
    ).count()
    ...
    monthly_stats = {
        ...
        "completed_courses": completed_trackings.filter(
            actual_end_date__gte=last_month
        ).count(),
        "attendance_rate": round(
            (
                AttendanceRecord.objects.filter(
                    attendance__date__gte=last_month, student_attend=True
                ).count()
                / AttendanceRecord.objects.filter(
    ...
    context = {
        **custom_admin_site.each_context(
        ...
        "attendance_rate": round(
            (today_present / today_attendance * 100) if today_attendance > 0 else 0, 1
        ),
        # Recent Feedback
...
def studentDashboard(request):
    ...
    # Recent Leave Requests
    recent_leaves = Student_Leave.objects.filter(student=student).order_by(
        "-created_at"
    )[:5]

    # Recent Feedback
    recent_feedback = StudentFeedback.objects.filter(student=student).order_by(
        "-created_at"
    )[:5]
...
def teacherDashboard(request):
    ...
    today_routines = Routine.objects.filter(
        teacher=teacher, attendances__date=today
    ).order_by("start_time")
    ...
    total_students = (
        StudentRoutine.objects.filter(routine__teacher=teacher)
        .values("student")
        .distinct()
        .count()
    )
